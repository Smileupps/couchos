...
/*  vm.update = function(){
    try {
    $('#test').val(JSON.stringify($('.app:visible').my('data').doc.rewrites.slice(3),null,4));
  } catch(e){};
    setTimeout(vm.update,500);
  };
...
*/
  ...
  $scope.newDdoc=function(){
    ...
    happy.addOrUpdateToDocs({
        ...
        "README.md":"# Welcome to your first Design Document!\n\nButtons on the right are used to create new fields, edit json, upload file/zip from your local disk, or zipped Github repositories.\n\nReserved fields,such as *views*, *lists*, *shows*, *rewrites* and *_attachments*, are displayed with an icon on the left. Click on these icons, for help pages.\n\nTo modify an attachment (within *_attachments* field), please click on *Download attachment*, then edit its *data* field.\n",
        language : "javascript",
        _attachments:{
          'index.html':{
  ...
  /*$scope.test=function(){
    var d = happy.getDoc('smileupps/bootstrap-theme');
    d["README.md"]=(Math.random()*100).toString();
  }
*/
  ...
  $scope.confirmDeleteDoc=function(id){
    if ((id.split("/")[1]||"").indexOf("_")==0){
      happy.err("You can't delete ddoc starting with underscore. These are system ddocs. If you really want to drop them, please use the Futon control panel.");
    } else { 
          ...
          var flat = function(o) {
            ...
            for (name in o) {
              if (o.hasOwnProperty(name)) {
                (typeof o[name] === "object"&&window.depthOf(o[name])>=2) ? flat(o[name], prefix + name + "/", out) : out[prefix + name] = o[name];
              }
            }
            return out;
          };
          if (doc._attachments) doc._attachments = flat(doc._attachments);
          return doc;
        },
  ...
  window.saveDdoc =function($ed) {
    ...
    if (!vm.saving) {
      vm.saving=true;
      $ed.addClass('disabled').parent().find('.loading').show();
      window.setTimeout(function(){
        vm.performSave($ed).then(function(to){
          $ed.removeClass('disabled').parent().find('.loading').hide();
          vm.saving=false;
          happy.success('Successfully Saved! If you just uploaded many attachments, please refresh this page if you don\'t want to push them again at next requests');
        },function(data){
          $ed.removeClass('disabled').parent().find('.loading').hide();
          vm.saving=false;
          happy.err(data);
      ...
      var isfresh=mydata.oldid.indexOf("/")==0,
          ...
          loadNewDoc=function(){
            ...
            happy.loadDoc(getFullId(to),true).then(function(data){
              ...
              //delete data.__id;
              //delete data.$$hashKey;   //data
              $ed.my('data',{oldid:to, doc:{_rev:data._rev}});
  ...
  vm.getBase64_Native_Faster_but_CallStackExceeds=function(arrayBuffer){
  ...
  window.mybtoa=function(str) {
    return window.btoa(unescape(encodeURIComponent(str)));
  };

  window.myatob=function(str) {
...
myApp.controller('LoginCtrl',['$scope','happy',function($scope,happy) {
  ...
  vm.login = function (email) {
    happy.action('signin', angular.extend({},$scope.fields,{
        ...
        happy.session().then(happy.loadDbs);
